...
Before deploying, install and configure the following tools:
...
Before deploying the backend, you need a Cognito User Pool:

1. Go to **AWS Console → Cognito → Create user pool**
2. Sign-in option: **Email**
3. Password policy: default
4. MFA: **No MFA**
5. Email delivery: **Send email with Cognito** (for testing)
6. App client: create one, note the **Client ID**
...
| Parameter | Value |
|---|---|
| Stack name | `tribunet-prod` |
| AWS Region | `us-east-1` |
| CognitoUserPoolId | your User Pool ID from Step 2 |
| CognitoClientId | your App Client ID from Step 2 |
| Stage | `prod` |
| AlertEmail | your email (for CloudWatch alerts) |
| AlertEmailSecondary | second email (optional) |
...
> **Note:** SAM will send a confirmation email to both alert addresses — confirm the subscription to activate CloudWatch alerts.
...
This loads 7 Israeli football leagues and 26 teams into DynamoDB.
...
Edit `frontend/.env` with the values from the previous steps:
...
> Edit `bucket-policy.json` first — replace `YOUR-BUCKET-NAME` with your actual bucket name.
...
Update `frontend/.env` with the CloudFront URL if needed, then rebuild:
...
### Step 7 — Set Up Amazon Location Service
...
### Step 8 — Set the First Admin User
